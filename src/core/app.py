...
class EduAIApp(QMainWindow):
    ...
    def _create_profile_page(self):
        """Cria a página de perfil do usuário"""
        try:
            from ..ui.profile_widget import ProfileWidget
            ...
            if not user_data:
                # Criar dados padrão se não encontrar
                user_data = {
                    'nome': self.user_name,
                    'email': '',
                    'perfil': 'educador',
            ...
            # Criar o widget de perfil
            profile_widget = ProfileWidget(self.user_name, user_data)
            
            # Conectar o sinal de voltar ao dashboard
            profile_widget.back_to_dashboard.connect(self._back_to_dashboard)
            
            return profile_widget
            
        except Exception as e:
            print(f"ERRO ao criar página de perfil: {e}")
            import traceback
            traceback.print_exc()
            # Retornar um widget de erro
            error_widget = QWidget()
            error_layout = QVBoxLayout(error_widget)
            error_label = QLabel(f"Erro ao carregar perfil: {e}")
