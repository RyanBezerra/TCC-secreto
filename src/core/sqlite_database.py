"""
EduAI - Gerenciamento de Banco de Dados SQLite
Sistema de conexão e operações com SQLite para desenvolvimento local
"""
...
class SQLiteDatabaseManager:
    ...
    def __init__(self):
        if not SQLiteDatabaseManager._initialized:
            # Configurações de conexão SQLite
            self.db_path = Path(__file__).parent.parent.parent / "database" / "eduai_local.db"
    ...
    def _ensure_tables(self):
        ...
        try:
            ...
            logger.info("Tabelas SQLite verificadas e criadas com sucesso")
        except Exception as e:
            logger.error(f"Erro ao verificar/criar tabelas SQLite: {e}")
    
    @contextmanager
    def get_connection(self):
        """Context manager para obter conexão SQLite"""
        connection = None
        try:
            connection = sqlite3.connect(str(self.db_path))
            connection.row_factory = sqlite3.Row  # Para retornar dicionários
            connection.execute("PRAGMA foreign_keys = ON")  # Habilitar foreign keys
            yield connection
        except sqlite3.Error as e:
            logger.error(f"Erro ao obter conexão SQLite: {e}")
            raise
        finally:
            if connection:
                connection.close()
        ...
        with LogOperation(f"Query: {query[:50]}..."):
            try:
                with self.get_cursor() as cursor:
                    ...
                    logger.info(f"SQLite SELECT operation successful - returned {len(results)} rows")
                    return [dict(row) for row in results]
            except sqlite3.Error as e:
                logger.error(f"SQLite SELECT operation failed: {str(e)}")
                return []
    ...
    def execute_update(self, query: str, params: tuple = None) -> bool:
        ...
        with LogOperation(f"Update: {query[:50]}..."):
            try:
                with self.get_cursor() as cursor:
                    cursor.execute(query, params or ())
                    logger.info("SQLite UPDATE operation successful")
                    return True
            except sqlite3.Error as e:
                logger.error(f"SQLite UPDATE operation failed: {str(e)}")
                return False
    ...
    def create_user(self, nome: str, idade: int, senha: str, nota: float = None, perfil: str = 'aluno') -> bool:
        """Cria um novo usuário no banco de dados"""
        senha_hash = self.hash_password(senha)
        query = """
            INSERT INTO usuario (nome, idade, senha_hash, nota, perfil, data_cadastro)
            VALUES (?, ?, ?, ?, ?, ?)
        """
        params = (nome, idade, senha_hash, nota, perfil, datetime.now())
    ...
    def get_aula_by_id(self, aula_id: int) -> Optional[Dict]:
        """Busca aula pelo ID"""
        query = "SELECT * FROM aulas WHERE id_aula = ?"
    ...
    def ensure_aulas_embedding_column(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Erro ao verificar/adicionar coluna embedding_json: {e}")
            pass

    def update_aula_embedding_json(self, aula_id: int, vec: List[float]) -> bool:
        """Atualiza o embedding (JSON) de uma aula."""
        import json as _json
        query = "UPDATE aulas SET embedding_json = ? WHERE id_aula = ?"
    ...
    def search_aulas_by_tag(self, tag: str) -> List[Dict]:
        """Busca aulas por tag"""
        query = "SELECT * FROM aulas WHERE tags LIKE ? ORDER BY titulo"
        params = (f"%{tag}%",)
    ...
    def search_aulas_keyword(self, query_text: str, limit: int = 5) -> List[Dict]:
        ...
        for word in words:
            like_pattern = f"%{word}%"
            conditions.append("(titulo LIKE ? OR descricao LIKE ? OR tags LIKE ? OR legendas LIKE ?)")
        ...
        base_score_query = f"""
            SELECT *, 
                   (
                       ...
                       (CASE WHEN titulo LIKE ANY(ARRAY[{','.join(['?'] * len(words))}]) THEN 10 ELSE 0 END) +
                       (CASE WHEN descricao LIKE ANY(ARRAY[{','.join(['?'] * len(words))}]) THEN 3 ELSE 0 END) +
                       (CASE WHEN tags LIKE ANY(ARRAY[{','.join(['?'] * len(words))}]) THEN 5 ELSE 0 END) +
                       (CASE WHEN legendas LIKE ANY(ARRAY[{','.join(['?'] * len(words))}]) THEN 2 ELSE 0 END)
        """
        
        # Adicionar bonus para palavras específicas se existirem
        if specific_words:
            base_score_query += f" + (CASE WHEN titulo LIKE ANY(ARRAY[{','.join(['?'] * len(specific_words))}]) THEN 5 ELSE 0 END)"
        
        if very_specific_words:
            base_score_query += f" + (CASE WHEN titulo LIKE ANY(ARRAY[{','.join(['?'] * len(very_specific_words))}]) THEN 10 ELSE 0 END)"
    ...
    def get_monthly_search_counts(self, months: int = 6) -> List[Dict]:
        ...
        query = """
            SELECT strftime('%m', h.timestamp) AS mes,
                   strftime('%Y-%m', h.timestamp) AS mes_data,
                   COUNT(*) AS total
            FROM historico h
    ...
    def update_suggestion_status(self, suggestion_id: int, status: str, feedback: str = None) -> bool:
        ...
        query = """
            ...
            SET status = ?, feedback = ?, data_avaliacao = ?
            WHERE id = ?
        """
    ...
    def test_connection(self) -> bool:
        ...
        try:
            with self.get_cursor() as cursor:
                ...
                logger.info("Teste de conexão com banco SQLite: SUCESSO")
                return result is not None
        except sqlite3.Error as e:
            logger.error(f"Erro ao testar conexão SQLite: {e}")
            return False
        ...
        try:
            with self.get_cursor() as cursor:
                ...
                if not table_exists:
                    ...
                    """)
                    logger.info("Tabela feedback criada com sucesso")
                else:
                    logger.info("Tabela feedback já existe")
        except Exception as e:
    ...
    def get_user_feedback_count(self, user_id: int) -> int:
        ...
        query = "SELECT COUNT(*) as total FROM feedback WHERE id_usuario = ?"
        results = self.execute_query(query, (user_id,))
    ...
    def get_instituicao_by_id(self, instituicao_id: int) -> Optional[Dict]:
        ...
        query = "SELECT * FROM instituicoes WHERE id = ?"
    ...
    def get_instituicao_by_cnpj(self, cnpj: str) -> Optional[Dict]:
        ...
        query = "SELECT * FROM instituicoes WHERE cnpj = ?"
    ...
    def update_instituicao(self, instituicao_id: int, instituicao_data: Dict) -> bool:
        ...
        query = f"UPDATE instituicoes SET {', '.join(updates)} WHERE id = ?"
        return self.execute_update(query, tuple(params))
    
    def delete_instituicao(self, instituicao_id: int) -> bool:
        """Remove uma instituição"""
        query = "DELETE FROM instituicoes WHERE id = ?"
            ...
            with self.get_cursor() as cursor:
                ...
                if not table_exists:
                    ...
                    if 'perfil' not in columns:
                        ...
                        cursor.execute("""
                            ALTER TABLE usuario 
                            ADD COLUMN perfil VARCHAR(20) DEFAULT 'aluno'
                        """)
...
# Instância global do gerenciador de banco de dados SQLite
